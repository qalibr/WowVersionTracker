    ...
    return (
        <div className="flex flex-wrap gap-3 mb-4">
            {filteredData.map((row, index) => {
                const cardId = `${product}-${row.region}-${row.version_name}`;
                const isSelected = !!selectedCards[cardId];

                return (
                    <div
                        key={index}
                        onClick={() => onToggle(product, row.region, row.version_name)}
                    ...
                    >
                        ...
                        <div className="mb-2">
                            <span className="text-lg font-mono font-bold block leading-none" title={row.version_name}>
                                {row.version_name}
                            </span>
                        </div>

                        <div className="text-[11px] opacity-70 mt-auto pt-2 border-t border-base-content/10 flex justify-between">
                            <span>Build: <span className="font-semibold text-base-content">{row.build_id}</span></span>
                            <span className="font-mono">Cfg: {row.build_config ? row.build_config.substring(0, 8) : 'N/A'}</span>
                        </div>
                    </div>
                );
            })}
